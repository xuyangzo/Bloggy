      ...
      if (postLeft < 3) {
        this.setState(
          ...
          () => {
            ...
            if (postLeft != 0) {
              const postString = this.state.postList
                .slice(currRender, currRender + postLeft)
                .join(",");
              const postObj = {
                posts: postString
              };
              axios
                .post("/api/posts/certain", postObj)
                .then(res => {
      ...
      () => {
        if (navName == "Blogs") {
          ...
          axios
            .get("/api/users/" + this.props.userid)
            .then(res => {
              const postList = res.data.favorites.map(obj => obj._id);
              this.setState({
                postList: postList,
                renderCount: 0
              });
              ...
              <ul className="navbar-nav">
                <li className="nav-item" style={{ cursor: "pointer" }}>
                  <a
                    ...
                    onClick={() => this.handleNavClick("Blogs")}
                  >
                    Blogs
                  </a>
                </li>
                <li class="nav-item" style={{ cursor: "pointer" }}>
                  <a
                    ...
                    onClick={() => this.handleNavClick("Favorites")}
                  >
                    Favorites
                  </a>
                </li>
                  ...
                  >
                    <div className="featured-place-wrap">
                      <div className="featured-title-box post-preview">
                        ...
                        <img
                          src="https://cdn-images-1.medium.com/max/2000/1*GEwEeymxbjn7weKXGjq48Q.png"
                          className="img-fluid"
                          alt="#"
                        />
                        <h3 className="custom post-title">{post.title}</h3>
                        <h4 className="custom post-subtitle">
                          {post.subtitle}
                        </h4>
